...
class LexRankSummarizer(AbstractSummarizer):
    ...
    def __call__(self, document,h,sentences_count):
        ...
        # extract stop words from the file english.txt
        ...
        # Stemming using WordNet Lemmatizer 
        sw=list()
        hw=list()
        
        for i in range(0,len(sentences_words)):
            sw.append(ma.ms(' '.join(sentences_words[i])))
            #print (set(sw[i]+sentences_words[i]))

        sentences_words=sw
        
        #h='Computer virus malware'
        
        head_words = [self._to_words_set(s) for s in h.sentences]

        for i in range(0,len(head_words)):
            hw.append(ma.ms(' '.join(head_words[i])))
        ...
        idf_metrics,rel_sq = self._compute_idf(sentences_words,tf_metrics,head_words)

        matrix1 = self._create_matrix(sentences_words, self.threshold, tf_metrics, idf_metrics)
        matrix2 = self._create_rel_matrix(sentences_words, rel_sq)
        
        scores = self.power_method((matrix1*(1-d))+(matrix2*d), self.epsilon) # TS LR

        scores_a = self.power_method(matrix1, self.epsilon) # LR
        ...
        return self._get_best_sentences(sum(scores/max(scores)),document.sentences, sentences_count, ratings)
    ...
    def _compute_idf(self,sentences,tf_metrics,s):
        idf_metrics = {}
        ...
        for sentence in sentences:
            for term in sentence:
                if term not in idf_metrics:
                    ...
                    idf_metrics[term] = math.log((sentences_count+1) / (0.5 + n_j))

        #print (idf_metrics)
        
        for sentence in sentences:
            t=0
            for term in list(set(sentence)):
                if term.lower()in s:
                    #print (s.count(term))
                    t+=idf_metrics[term]*math.log(s.count(term)+1)*math.log(tf_metrics[i][term]+1)
            rel_sq.append(t)        
            i+=1
